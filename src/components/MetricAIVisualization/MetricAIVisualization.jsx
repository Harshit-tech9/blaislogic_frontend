...
import { NetworkPhysics } from './networkPhysics'

export default function MetricAIVisualization() {
  ...
  useEffect(() => {
    ...
    const setup = () => {
      ...
      if (!physicsRef.current) {
        physicsRef.current = new NetworkPhysics(w, h)
  ...
  return (
    <div ref={containerRef} style={{ width: '100%', height: '100%', position: 'relative' }}>
      ...
      {/* Hover Card */}
      {hoveredNode && hoveredNode.id !== 'metricai' && (
        <div 
          style={{
            position: 'absolute',
        ...
        >
          <div style={{ fontSize: '10px', textTransform: 'uppercase', color: 'var(--copy)', fontWeight: 600, letterSpacing: '0.05em' }}>
            {hoveredNode.label}
          </div>
          <div style={{ fontSize: '16px', fontWeight: 600, color: 'var(--heading)', marginTop: '4px' }}>
            {hoveredNode.val}
          </div>
        </div>
      )}
